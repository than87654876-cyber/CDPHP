...
@section('title', 'Thêm Món ăn - FOODDAILY Admin')
...
<div class="max-w-4xl mx-auto space-y-6">
    ...
    <div class="flex items-center justify-between">
        <div>
            <h1 class="text-2xl font-extrabold text-slate-900 tracking-tight">Thêm Món Ăn Mới</h1>
            <p class="text-xs text-slate-500 font-medium mt-1">Tạo món ăn mới cho thực đơn của cửa hàng</p>
        </div>
    ...
    <div class="bg-white rounded-3xl p-6 sm:p-8 shadow-sm border border-slate-200/80 space-y-6">
        <div class="flex items-center gap-3 pb-4 border-b border-slate-100">
            ...
            <div>
                <h3 class="text-sm font-extrabold text-slate-900">Thông tin món ăn mới</h3>
                <p class="text-[11px] text-slate-400 font-medium">Nhập đầy đủ thông tin tên món, hình ảnh, đơn giá và phân loại</p>
            </div>
        </div>

        <form action="{{ route('monandon_them.post') }}" method="POST" enctype="multipart/form-data" class="space-y-6">
            @csrf
            ...
            <div class="grid grid-cols-1 sm:grid-cols-2 gap-6">
                ...
                <div class="space-y-2">
                    ...
                    <input type="text" id="dish_name" name="dish_name" value="{{ old('dish_name') }}" placeholder="Ví dụ: Phở bò đặc biệt, Cơm sườn trứng..." required class="w-full px-4 py-3 rounded-2xl border border-slate-200 text-xs font-semibold text-slate-900 focus:border-[#ee4d2d] outline-none transition-all shadow-xs">
                </div>
                ...
                <div class="space-y-2">
                    ...
                    <div class="relative">
                        <input type="number" step="1000" id="price" name="price" value="{{ old('price', '35000') }}" required min="0" class="w-full pl-4 pr-10 py-3 rounded-2xl border border-slate-200 text-xs font-bold text-[#ee4d2d] focus:border-[#ee4d2d] outline-none transition-all shadow-xs">
            ...
            <div class="space-y-3 pt-2">
                ...
                <div class="flex flex-col sm:flex-row items-center gap-6 p-4 rounded-2xl bg-slate-50 border border-slate-200">
                    <div class="w-24 h-24 rounded-2xl overflow-hidden bg-white border border-slate-200 shadow-xs flex-shrink-0">
                        <img id="dish-preview" src="{{ asset('logo.jpg') }}" alt="Dish preview" class="w-full h-full object-cover">
                    </div>
                    <div class="space-y-2 flex-grow w-full">
                        ...
                        <p class="text-[10px] text-slate-400 font-medium">Tải lên hình ảnh món ăn chất lượng cao (JPG, PNG, WEBP tối đa 5MB)</p>
                    </div>
                </div>
            </div>
            ...
            <div class="space-y-2">
                ...
                <textarea id="description" name="description" rows="3" placeholder="Nhập mô tả hương vị, nguyên liệu đặc trưng của món..." class="w-full p-4 rounded-2xl border border-slate-200 text-xs font-medium text-slate-900 focus:border-[#ee4d2d] outline-none transition-all shadow-xs leading-relaxed">{{ old('description') }}</textarea>
            </div>
            ...
            <div class="pt-4 border-t border-slate-100 flex items-center justify-end gap-3">
                ...
                <button type="submit" class="px-8 py-3 rounded-2xl bg-[#ee4d2d] hover:bg-red-600 text-white font-extrabold text-xs shadow-md shadow-rose-500/20 transition-all flex items-center gap-2 active:scale-98">
                    <i class="fas fa-plus"></i> THÊM MÓN VÀO THỰC ĐƠN
                </button>
            </div>
        </form>
    </div>
