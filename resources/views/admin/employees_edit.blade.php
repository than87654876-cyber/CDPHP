...
@section('title', 'Chỉnh sửa Nhân viên - FOODDAILY Admin')
...
<div class="max-w-4xl mx-auto space-y-6">
    ...
    <div class="flex items-center justify-between">
        <div>
            ...
            <p class="text-xs text-slate-500 font-medium mt-1">Điều chỉnh thông tin cá nhân, phân quyền và trạng thái tài khoản</p>
        </div>
        <a href="{{ route('nhanvien_xem', $employee->id) }}" class="px-4 py-2.5 rounded-xl bg-slate-100 hover:bg-slate-200 text-slate-700 font-bold text-xs transition-colors flex items-center gap-2">
    ...
    <div class="bg-white rounded-3xl p-6 sm:p-8 shadow-sm border border-slate-200/80 space-y-6">
        <div class="flex items-center gap-3 pb-4 border-b border-slate-100">
            ...
            <div>
                <h3 class="text-sm font-extrabold text-slate-900">Biểu mẫu điều chỉnh thông tin nhân sự</h3>
                <p class="text-[11px] text-slate-400 font-medium">Mã nhân viên cố định: <strong class="text-slate-800">NV-{{ sprintf('%03d', $employee->id) }}</strong></p>
            </div>
        </div>

        <form action="{{ route('nhanvien_chinhsua.post', $employee->id) }}" method="POST" class="space-y-6">
            @csrf
            ...
            <div class="grid grid-cols-1 sm:grid-cols-2 gap-6">
                ...
                <div class="space-y-2">
                    ...
                    <input type="text" id="fullname" name="fullname" value="{{ old('fullname', $employee->fullname) }}" required class="w-full px-4 py-3 rounded-2xl border border-slate-200 text-xs font-bold text-slate-900 focus:border-[#ee4d2d] outline-none transition-all shadow-xs">
                </div>
                ...
                <div class="space-y-2">
                    ...
                    <input type="text" id="phone" name="phone" value="{{ old('phone', $employee->phone) }}" required class="w-full px-4 py-3 rounded-2xl border border-slate-200 text-xs font-bold text-slate-900 focus:border-[#ee4d2d] outline-none transition-all shadow-xs">
                </div>
                ...
                <div class="space-y-2">
                    ...
                    <input type="email" id="email" name="email" value="{{ old('email', $employee->email) }}" required class="w-full px-4 py-3 rounded-2xl border border-slate-200 text-xs font-bold text-slate-900 focus:border-[#ee4d2d] outline-none transition-all shadow-xs">
                </div>
                ...
                <div class="space-y-2">
                    ...
                    <input type="password" id="password" name="password" placeholder="Nhập nếu muốn đổi mật khẩu" class="w-full px-4 py-3 rounded-2xl border border-slate-200 text-xs font-bold text-slate-900 focus:border-[#ee4d2d] outline-none transition-all shadow-xs">
                </div>
                ...
                <div class="space-y-2">
                    ...
                    <select id="role" name="role" required class="w-full px-4 py-3 rounded-2xl border border-slate-200 text-xs font-bold text-slate-900 focus:border-[#ee4d2d] outline-none transition-all shadow-xs bg-white">
                        <option value="staff" {{ old('role', $employee->role) === 'staff' ? 'selected' : '' }}>👨‍🍳 Nhân viên vận hành (Staff - Bàn làm việc & Bếp)</option>
                        <option value="admin" {{ old('role', $employee->role) === 'admin' ? 'selected' : '' }}>⭐ Quản trị viên (Admin - Quản lý thực đơn, đơn hàng, khách hàng)</option>
                        <option value="superadmin" {{ old('role', $employee->role) === 'superadmin' ? 'selected' : '' }}>👑 Quản trị viên tối cao (Super Admin - Toàn quyền & Phân quyền)</option>
                    </select>
                </div>
                ...
                <div class="space-y-2 sm:col-span-2">
                    ...
                    <input type="text" id="notes" name="notes" value="{{ old('notes', $employee->notes) }}" placeholder="Ví dụ: Bếp chính, Thu ngân ca sáng..." class="w-full px-4 py-3 rounded-2xl border border-slate-200 text-xs font-medium text-slate-900 focus:border-[#ee4d2d] outline-none transition-all shadow-xs">
                </div>
            </div>

            <div class="pt-4 border-t border-slate-100 flex items-center justify-end gap-3">
                <a href="{{ route('nhanvien_xem', $employee->id) }}" class="px-6 py-3 rounded-2xl bg-slate-100 hover:bg-slate-200 text-slate-700 font-bold text-xs transition-colors">
                    Hủy bỏ
                </a>
