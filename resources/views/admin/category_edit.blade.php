...
@section('title', 'Chỉnh sửa Danh mục - FOODDAILY Admin')
...
<div class="max-w-3xl mx-auto space-y-6">
    ...
    <div class="flex items-center justify-between">
        <div>
            <h1 class="text-2xl font-extrabold text-slate-900 tracking-tight">Chỉnh Sửa Danh Mục</h1>
            <p class="text-xs text-slate-500 font-medium mt-1">Cập nhật thông tin danh mục: <strong class="text-[#ee4d2d]">{{ $category->category_name }}</strong></p>
        </div>
    ...
    <div class="bg-white rounded-3xl p-6 sm:p-8 shadow-sm border border-slate-200/80 space-y-6">
        <div class="flex items-center gap-3 pb-4 border-b border-slate-100">
            ...
            <div>
                <h3 class="text-sm font-extrabold text-slate-900">Biểu mẫu cập nhật danh mục</h3>
                <p class="text-[11px] text-slate-400 font-medium">Chỉnh sửa tên danh mục và mô tả nhóm món ăn</p>
            </div>
        </div>

        <form action="{{ route('danhmuc_chinhsua.post', $category->id) }}" method="POST" class="space-y-6">
            @csrf
            ...
            <div class="space-y-2">
                ...
                <input type="text" id="category_name" name="category_name" value="{{ old('category_name', $category->category_name) }}" required class="w-full px-4 py-3 rounded-2xl border border-slate-200 text-xs font-semibold text-slate-900 focus:border-[#ee4d2d] focus:bg-white outline-none transition-all shadow-xs">
            </div>
