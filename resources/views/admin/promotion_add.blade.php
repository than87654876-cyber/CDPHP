...
@section('title', 'Thêm Mã khuyến mãi - FOODDAILY Admin')
...
<div class="max-w-4xl mx-auto space-y-6">
    ...
    <div class="flex items-center justify-between">
        <div>
            <h1 class="text-2xl font-extrabold text-slate-900 tracking-tight">Thêm Mã Khuyến Mãi Mới</h1>
            <p class="text-xs text-slate-500 font-medium mt-1">Tạo coupon chiết khấu và ưu đãi mới cho khách hàng</p>
        </div>
    ...
    <div class="bg-white rounded-3xl p-6 sm:p-8 shadow-sm border border-slate-200/80 space-y-6">
        <div class="flex items-center gap-3 pb-4 border-b border-slate-100">
            ...
            <div>
                <h3 class="text-sm font-extrabold text-slate-900">Biểu mẫu phát hành coupon</h3>
                <p class="text-[11px] text-slate-400 font-medium">Nhập thông tin mã giảm giá để áp dụng trên toàn hệ thống</p>
            </div>
        </div>

        <form action="{{ route('khuyenmai_them.post') }}" method="POST" class="space-y-6">
            @csrf
            ...
            <div class="grid grid-cols-1 sm:grid-cols-2 gap-6">
                ...
                <div class="space-y-2">
                    ...
                    <input type="text" id="coupon_code" name="coupon_code" value="{{ old('coupon_code') }}" required placeholder="Ví dụ: SUMMER2026, GIAM20K" class="w-full px-4 py-3 rounded-2xl border border-slate-200 text-xs font-black uppercase text-slate-900 focus:border-[#ee4d2d] outline-none transition-all shadow-xs">
                </div>
                ...
                <div class="space-y-2">
                    ...
                    <input type="number" step="1" min="0" id="discount_value" name="discount_value" value="{{ old('discount_value') }}" required placeholder="Ví dụ: 20 (nếu là %) hoặc 20000 (nếu là tiền)" class="w-full px-4 py-3 rounded-2xl border border-slate-200 text-xs font-black text-rose-600 focus:border-[#ee4d2d] outline-none transition-all shadow-xs">
                </div>
                ...
                <div class="space-y-2">
                    ...
                    <input type="number" step="1" min="0" id="min_order_value" name="min_order_value" value="{{ old('min_order_value', 0) }}" required class="w-full px-4 py-3 rounded-2xl border border-slate-200 text-xs font-semibold text-slate-900 focus:border-[#ee4d2d] outline-none transition-all shadow-xs">
                </div>
                ...
                <div class="space-y-2">
                    ...
                    <input type="date" id="start_date" name="start_date" value="{{ old('start_date', date('Y-m-d')) }}" required class="w-full px-4 py-3 rounded-2xl border border-slate-200 text-xs font-semibold text-slate-900 focus:border-[#ee4d2d] outline-none transition-all shadow-xs bg-white">
                </div>
                ...
                <div class="space-y-2">
                    ...
                    <input type="date" id="end_date" name="end_date" value="{{ old('end_date', date('Y-m-d', strtotime('+30 days'))) }}" required class="w-full px-4 py-3 rounded-2xl border border-slate-200 text-xs font-semibold text-slate-900 focus:border-[#ee4d2d] outline-none transition-all shadow-xs bg-white">
                </div>
                ...
                <div class="space-y-2 sm:col-span-2">
                    ...
                    <input type="number" min="1" id="usage_limit" name="usage_limit" value="{{ old('usage_limit') }}" placeholder="Ví dụ: 100 (Để trống = Vô hạn)" class="w-full px-4 py-3 rounded-2xl border border-slate-200 text-xs font-semibold text-slate-900 focus:border-[#ee4d2d] outline-none transition-all shadow-xs">
                </div>
            </div>
            ...
            <div class="pt-4 border-t border-slate-100 flex items-center justify-end gap-3">
                ...
                <button type="submit" class="px-8 py-3 rounded-2xl bg-[#ee4d2d] hover:bg-red-600 text-white font-extrabold text-xs shadow-md shadow-rose-500/25 hover:scale-[1.01] active:scale-[0.99] transition-all cursor-pointer">
                    <i class="fas fa-plus mr-1.5"></i> Tạo mã khuyến mãi
                </button>
            </div>
        </form>
    </div>
